<nav class="bg-navy-500 dark:bg-navy-700 border-b border-navy-600 dark:border-navy-800 transition-colors duration-200"
    x-data="{ open: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <!-- Search (desktop) -->
                <form action="{{ route('shop.index') }}" method="GET" class="hidden sm:flex items-center ml-6">
                    <label for="nav-search" class="sr-only">Rechercher</label>
                    <div class="relative">
                        <input id="nav-search" name="q" type="search" placeholder="Rechercher des produits..."
                            value="{{ request('q') }}"
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-6 space-x-2">
                ...
                <a href="{{ route('shop.cart') }}"
                    ...
                    @if ($cartCount > 0)
                        <span
                            class="absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-navy-900 transform translate-x-1/2 -translate-y-1/2 bg-gold-500 rounded-full">
                            {{ $cartCount }}
                        </span>
                    @endif
                </a>
                ...
                @else
                    ...
                    <a href="{{ route('register') }}"
                        class="ml-4 text-sm text-gray-500 hover:text-gray-900">Inscription</a>
                @endauth
            </div>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        ...
        <div class="px-4 pb-4">
            <form action="{{ route('shop.index') }}" method="GET" class="flex">
                ...
                <input id="mobile-search" name="q" type="search" placeholder="Rechercher des produits..."
                    value="{{ request('q') }}"
        ...
        <div class="pt-2 pb-3 space-y-1">
            <a href="{{ route('home') }}"
                class="block pl-3 pr-4 py-2 border-l-4 {{ request()->routeIs('home') ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300' }} text-base font-medium focus:outline-none transition duration-150 ease-in-out">
                Accueil
            </a>
            <a href="{{ route('about') }}"
                class="block pl-3 pr-4 py-2 border-l-4 {{ request()->routeIs('about') ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300' }} text-base font-medium focus:outline-none transition duration-150 ease-in-out">
                À propos
            </a>
            <a href="{{ route('services.index') }}"
                class="block pl-3 pr-4 py-2 border-l-4 {{ request()->routeIs('services.*') ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300' }} text-base font-medium focus:outline-none transition duration-150 ease-in-out">
                Services
            </a>
            <a href="{{ route('portfolio.index') }}"
                class="block pl-3 pr-4 py-2 border-l-4 {{ request()->routeIs('portfolio.*') ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300' }} text-base font-medium focus:outline-none transition duration-150 ease-in-out">
                Portfolio
            </a>
            <a href="{{ route('shop.index') }}"
                class="block pl-3 pr-4 py-2 border-l-4 {{ request()->routeIs('shop.*') ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300' }} text-base font-medium focus:outline-none transition duration-150 ease-in-out">
                Boutique
            </a>
            <a href="{{ route('blog.index') }}"
                class="block pl-3 pr-4 py-2 border-l-4 {{ request()->routeIs('blog.*') ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300' }} text-base font-medium focus:outline-none transition duration-150 ease-in-out">
                Blog
            </a>
            <a href="{{ route('contact.index') }}"
                class="block pl-3 pr-4 py-2 border-l-4 {{ request()->routeIs('contact.*') ? 'border-indigo-400 text-indigo-700 bg-indigo-50' : 'border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300' }} text-base font-medium focus:outline-none transition duration-150 ease-in-out">
                Contact
            </a>
