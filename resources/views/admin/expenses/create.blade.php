...
@section('title', 'Enregistrer une Dépense - Admin')
...
@section('content')
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 mt-2">Enregistrer une Dépense</h1>
        <p class="text-gray-500 text-sm mt-1">Créez un nouvel enregistrement de dépense et débitez le compte bancaire sélectionné si applicable.</p>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-4xl" x-data="{ paymentMethod: 'cash' }">
        <form action="{{ route('admin.expenses.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Ex: Achat cartouches d'encre HP, Loyer Juillet 2026..." class="w-full rounded-lg border-gray-200 text-sm focus:ring-navy-500 focus:border-navy-500 @error('title') border-red-300 ring-1 ring-red-300 @enderror">
                    @error('title')
                ...
                <div>
                    ...
                    <div class="relative rounded-lg shadow-sm">
                        <input type="number" name="amount" id="amount" value="{{ old('amount') }}" step="0.01" min="0.01" required placeholder="0.00" class="w-full rounded-lg border-gray-200 text-sm focus:ring-navy-500 focus:border-navy-500 @error('amount') border-red-300 ring-1 ring-red-300 @enderror">
                    </div>
                    @error('amount')
                        <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="expense_date" id="expense_date" value="{{ old('expense_date', date('Y-m-d')) }}" required class="w-full rounded-lg border-gray-200 text-sm focus:ring-navy-500 focus:border-navy-500 @error('expense_date') border-red-300 ring-1 ring-red-300 @enderror">
                    @error('expense_date')
                        <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method" x-model="paymentMethod" required class="w-full rounded-lg border-gray-200 text-sm focus:ring-navy-500 focus:border-navy-500 @error('payment_method') border-red-300 ring-1 ring-red-300 @enderror">
                        @foreach(\App\Models\Expense::PAYMENT_METHODS as $key => $value)
                            <option value="{{ $key }}" {{ old('payment_method', 'cash') === $key ? 'selected' : '' }}>{{ $value }}</option>
                        @endforeach
                    </select>
                ...
                <!-- Bank Account Selection (Hidden if Cash or Other) -->
                ...
                <!-- Attachment Piece -->
                <div class="col-span-1 md:col-span-2">
                    <label for="attachment" class="block text-sm font-semibold text-gray-700 mb-1.5">Pièce justificative (Facture, Reçu, PDF... Max 5Mo)</label>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" placeholder="Détails supplémentaires concernant la dépense..." class="w-full rounded-lg border-gray-200 text-sm focus:ring-navy-500 focus:border-navy-500 @error('description') border-red-300 ring-1 ring-red-300 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                    @enderror
                </div>
