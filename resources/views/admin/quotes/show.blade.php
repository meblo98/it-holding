...
@section('title', 'Détails du Devis ' . $quote->number)

@section('content')
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Devis {{ $quote->number }}</h1>
        <p class="text-gray-500">Créé le {{ $quote->created_at->format('d/m/Y') }}</p>
    </div>
    <div class="flex flex-wrap gap-2">
        <a href="{{ route('admin.quotes.print', $quote->id) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
            Imprimer
        </a>
        <a href="{{ route('admin.quotes.share', $quote->id) }}" class="inline-flex items-center px-4 py-2 bg-navy-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-navy-700 active:bg-navy-900 focus:outline-none focus:ring-2 focus:ring-navy-500 focus:ring-offset-2 transition ease-in-out duration-150">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path></svg>
            Partager
        </a>
        @if($quote->status !== 'converted')
        <form action="{{ route('admin.quotes.convert', $quote->id) }}" method="POST" onsubmit="return confirm('Convertir ce devis en facture ?')">
            @csrf
        ...
        @endif
        <a href="{{ route('admin.quotes.edit', $quote->id) }}" class="inline-flex items-center px-4 py-2 bg-gold-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gold-700 active:bg-gold-900 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:ring-offset-2 transition ease-in-out duration-150">
            Modifier
        </a>
        <form action="{{ route('admin.quotes.destroy', $quote->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce devis ?')" class="inline">
            @csrf
            @method('DELETE')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1 space-y-6">
        <div class="bg-white shadow-sm rounded-lg p-6">
            ...
            <div class="space-y-3">
                <p class="text-sm font-bold text-gray-900">{{ $quote->client_name }}</p>
                ...
                <div class="text-sm text-gray-600 flex items-start">
                    ...
                    <span class="whitespace-pre-line">{{ $quote->client_address }}</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white shadow-sm rounded-lg p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">Valide jusqu'au:</span>
                    <span class="text-sm font-medium text-gray-900">{{ $quote->valid_until ? $quote->valid_until->format('d/m/Y') : 'N/A' }}</span>
                </div>
                @if($quote->invoice)
                <div class="border-t pt-3 flex justify-between items-center">
                    <span class="text-sm text-gray-500">Facture liée:</span>
                    <a href="{{ route('admin.invoices.show', $quote->invoice->id) }}" class="text-sm font-bold text-gold-600 hover:text-gold-900">{{ $quote->invoice->number }}</a>
                </div>
                @endif
            </div>
        </div>
