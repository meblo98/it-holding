...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <h1 class="text-3xl font-bold text-gray-900">Modifier {{ $quote->number }}</h1>
    <a href="{{ route('admin.quotes.show', $quote->id) }}" class="text-navy-600 hover:text-navy-900 underline font-medium">Voir les détails</a>
</div>
...
<form action="{{ route('admin.quotes.update', $quote->id) }}" method="POST" x-data="{ 
    ...
    applyTax: {{ $quote->tax_amount > 0 ? 'true' : 'false' }},
    ...
    @csrf
    @method('PUT')
    <input type="hidden" name="client_id" x-ref="clientId" value="{{ old('client_id', $quote->client_id) }}">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <div class="mb-4">
                    ...
                    <select id="select_client" @change="onClientChange($event)" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gold-500 focus:border-gold-500 sm:text-sm">
                        ...
                        @foreach($clients as $c)
                            <option value="{{ $c->id }}" {{ $quote->client_id == $c->id ? 'selected' : '' }} data-name="{{ $c->display_name }}" data-email="{{ $c->email }}" data-phone="{{ $c->phone }}" data-address="{{ $c->address }}">{{ $c->display_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="client_name" id="client_name" x-ref="clientName" required value="{{ old('client_name', $quote->client_name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gold-500 focus:border-gold-500 sm:text-sm">
                    </div>
                    <div>
                        <label for="client_email" class="block text-sm font-medium text-gray-700">Email</label>
                        <input type="email" name="client_email" id="client_email" x-ref="clientEmail" value="{{ old('client_email', $quote->client_email) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gold-500 focus:border-gold-500 sm:text-sm">
                    </div>
                    <div>
                        <label for="client_phone" class="block text-sm font-medium text-gray-700">Téléphone</label>
                        <input type="text" name="client_phone" id="client_phone" x-ref="clientPhone" value="{{ old('client_phone', $quote->client_phone) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gold-500 focus:border-gold-500 sm:text-sm">
                    </div>
                    <div>
                        <label for="valid_until" class="block text-sm font-medium text-gray-700">Valide jusqu'au</label>
                        <input type="date" name="valid_until" id="valid_until" value="{{ old('valid_until', $quote->valid_until ? $quote->valid_until->format('Y-m-d') : '') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gold-500 focus:border-gold-500 sm:text-sm">
                    </div>
                    <div class="md:col-span-2">
                        <label for="client_address" class="block text-sm font-medium text-gray-700">Adresse</label>
                        <textarea name="client_address" id="client_address" x-ref="clientAddress" rows="2" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gold-500 focus:border-gold-500 sm:text-sm">{{ old('client_address', $quote->client_address) }}</textarea>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <input type="text" name="number" id="number" value="{{ old('number', $quote->number) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gold-500 focus:border-gold-500 sm:text-sm font-bold text-navy-600">
                    </div>
                    <div>
            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                ...
                <textarea name="notes" id="notes" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-gold-500 focus:border-gold-500 sm:text-sm">{{ old('notes', $quote->notes) }}</textarea>
            </div>
        </div>
    </div>
</form>
